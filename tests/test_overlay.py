...
from videocortex.overlay import (
    OverlayError,
    hold_schedule,
    load_run,
    overlay_from_run,
...
from videocortex.render import format_pip_label, rounded_rect_alpha
...
def test_hold_schedule_does_not_overrun_eof():
    ts = np.arange(32, dtype=float)
    start, durs = hold_schedule(ts, video_duration=31.098, lag_s=0.0)
    assert start == 0.0
    assert durs[-1] == pytest.approx(0.098, abs=1e-9)
    assert start + float(durs.sum()) == pytest.approx(31.098)


def test_target_fps_uses_frame_count_not_tagged_60():
    fps = target_fps(
        {
            "nb_frames": "970",
            "avg_frame_rate": "9700/311",
            "r_frame_rate": "60/1",
        },
        duration=31.098333,
    )
    assert 30.0 < fps < 32.0
...
def test_compose_ffmpeg_overlays_a_tiny_clip(tmp_path):
    from videocortex.overlay import compose_ffmpeg, _write_blank_png

    src = tmp_path / "src.mp4"
    subprocess.run(
        [
            "ffmpeg", "-y", "-f", "lavfi", "-i", "color=c=blue:s=640x360:d=1:r=30",
...
@pytest.mark.skipif(not shutil.which("ffmpeg"), reason="ffmpeg not on PATH")
def test_compose_ffmpeg_keeps_audio_for_the_whole_clip(tmp_path):
    from videocortex.overlay import compose_ffmpeg, _write_blank_png
    ...
    compose_ffmpeg(
        video=src, concat=concat, box=(10, 10, 80, 72),
        out=out, has_audio=True, fast=False, crf=28,
    ...
    assert any(
        line.startswith("duration=") and float(line.split("=", 1)[1]) >= 1.8
    ...
    dts = [float(x) for x in pkt.stdout.splitlines() if x.strip()]
    assert dts
    # Packet PTS can rewind because of B-frames; DTS must not.
    assert all(b + 1e-6 >= a for a, b in zip(dts, dts[1:]))


@pytest.mark.slow
def test_render_pip_card_is_rgba_with_clear_corners(tmp_path):
    from videocortex.config import FSAVERAGE5_VERTICES, RenderConfig
    from videocortex.render import render_pip_card
...
def test_pose_index_wraps_and_respects_dps():
    from videocortex.spin import clamp_dps, n_poses, pose_blend, pose_index
...
def test_vertex_palette_quiet_stays_quieter_than_loud():
    from videocortex.spin import vertex_palette
...
def test_composite_smooth_blends_two_poses():
    from videocortex.spin import SpinAtlas, composite_frame_smooth
...
def test_composite_frame_lookup_and_alpha():
    from videocortex.spin import SpinAtlas, composite_frame
...
def test_iter_spin_coalesces_identical_keys():
    from videocortex.config import OverlayConfig
    from videocortex.spin import SpinAtlas, iter_spin_frames
...
def test_bake_atlas_ids_and_transparency(tmp_path):
    from videocortex.spin import bake_atlas, composite_frame, vertex_palette
