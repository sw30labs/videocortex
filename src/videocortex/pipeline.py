...
from videocortex.config import RenderConfig, RunConfig, write_manifest
from videocortex.patches import whisperx_cpu_compat
...
def predict(run: RunConfig) -> tuple[np.ndarray, list[float]]:
    ...
    """
    from videocortex.model import load_model
...
def run(run_cfg: RunConfig, render_cfg: RenderConfig) -> RunResult:
    ...
    if run_cfg.save_predictions:
        ...
        # matrix means `videocortex draw` can retry the picture for free.
