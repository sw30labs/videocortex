...
a twenty-gigabyte download or a forty-minute inference run: a gated model you
haven't accepted, a missing ffmpeg, torch built for the wrong architecture.
``videocortex doctor`` finds them in about five seconds.
"""
...
def check_torch() -> list[Check]:
    from videocortex.device import probe
    ...
    if info["has_cuda"]:
        checks.append(Check("accelerator", OK, "cuda available"))
    elif info["has_mps"]:
        checks.append(Check("accelerator", OK, "mps (Metal) available"))
    else:
        checks.append(
            Check(
                "accelerator", WARN,
                info["note"] or "CPU only — inference will be slow",
                "On Apple Silicon, confirm an arm64 python: "
...
def check_fsaverage5() -> Check:
    ...
    """
    if not _has("nilearn"):
        return Check("fsaverage5", FAIL, "nilearn not installed", "pip install -e .")
    try:
        from videocortex.render import load_fsaverage5
...
def check_hf_access(timeout: float = 10.0) -> list[Check]:
    ...
    for repo, probe in repos:
        ...
        # Gated: metadata being visible proves nothing. Try to actually read it.
        try:
