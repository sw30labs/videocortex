...
from videocortex import __version__
from videocortex.config import (
    HF_CHECKPOINT_REPO,
    OverlayConfig,
    RenderConfig,
    RunConfig,
...
from videocortex.web import DEFAULT_PORT


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="videocortex",
        description="Drop a clip. See what it would do to an average brain (Meta TRIBE v2).",
    )
    p.add_argument("--version", action="version", version=f"videocortex {__version__}")
    ...
    r.add_argument("--cache-dir", type=Path, default=Path(".videocortex-cache"))
    r.add_argument("--batch-size", type=int, default=1)
    r.add_argument("--feature-batch-size", type=int, default=1)
    ...
    o.add_argument("--fast", action="store_true", help="VideoToolbox encode")
...
def cmd_fetch(a: argparse.Namespace) -> int:
    from videocortex import weights
...
def cmd_render(a: argparse.Namespace) -> int:
    from videocortex import pipeline
...
def cmd_serve(a: argparse.Namespace) -> int:
    from videocortex.web.server import serve
...
def cmd_overlay(a: argparse.Namespace) -> int:
    from videocortex.overlay import OverlayError, overlay_from_run
