"""Loopback command deck for videocortex.
...
forty minutes of Metal, not something to queue behind.

Run:
    videocortex serve                 # http://127.0.0.1:8730
    python -m videocortex.web --port 9000
"""
...
from videocortex.web import DEFAULT_PORT, runner
    ...
    *,
    ...
    try:
        ...
        if exc.errno == errno.EADDRINUSE:
            ...
            print(f"(or pick another port: videocortex serve --port {port + 1})")
            raise SystemExit(1) from None
        raise

    url = f"http://{host}:{port}"
    print(f"videocortex command deck → {url}  (Ctrl+C to stop)", flush=True)
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="videocortex loopback command deck")
