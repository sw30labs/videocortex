...
the picture half; keeping them apart is what lets ``videocortex draw`` re-render
a saved prediction without torch anywhere in sight.
"""
...
class RunConfig:
    ...
    cache_dir: Path = Path(".videocortex-cache")
    #: Loader batch size. Upstream ships 8 with num_workers=20, tuned for a
    #: Slurm node. On a laptop that is a good way to get OOM-killed.
    batch_size: int = 1
    num_workers: int = 0
