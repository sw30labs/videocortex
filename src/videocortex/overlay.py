...
from videocortex.config import OverlayConfig
...
def hold_schedule(
    ...
    *,
    ...
    # Hold the last plate through EOF, but never past it. Forcing a full TR
    # here used to make concat ~1s longer than the source, so the muxed
    # MP4's video stream outran the audio.
    if ts.size == 1:
        last = max(1e-3, end - t_show[0])
        return start, np.array([last], dtype=np.float64)
    durs = np.diff(t_show)
    last = max(1e-3, end - t_show[-1])
    durs = np.append(durs, last)
...
def load_run(run_dir: Path) -> tuple[np.ndarray, np.ndarray, dict]:
    ...
    if not ts_path.is_file():
        raise OverlayError(
            ...
            "Re-run `videocortex render` (newer than the overlay feature)."
        )
    preds = np.load(pred)
...
def probe_video(path: Path) -> dict[str, tp.Any]:
    if shutil.which("ffprobe") is None:
        raise OverlayError("ffprobe not on PATH (comes with ffmpeg): brew install ffmpeg")
    cmd = [
        "ffprobe", "-v", "error",
        "-print_format", "json",
...
def _parse_rate(value: tp.Any) -> float | None:
    ...
    try:
        ...
        return None
    return rate if math.isfinite(rate) and rate > 1 else None


def target_fps(stream: dict, duration: float | None = None) -> float:
    """Constant frame rate to rebuild a watchable overlay clock.

    Zoom / screen-record MOVs often tag ``r_frame_rate=60`` while the
    packets are ~30fps with timestamps that go *backwards*. Copying AAC
    onto that clock is what makes players drop audio after a few seconds.
    """
    try:
        n_frames = int(stream.get("nb_frames") or 0)
    except (TypeError, ValueError):
        n_frames = 0
    if n_frames > 1 and duration and duration > 0:
        counted = n_frames / float(duration)
        if 8.0 <= counted <= 120.0:
            return float(counted)
    avg = _parse_rate(stream.get("avg_frame_rate"))
    nom = _parse_rate(stream.get("r_frame_rate"))
    fps = avg or nom or 30.0
    if nom and avg and nom > 45 and avg < nom * 0.75:
        fps = avg
    return float(min(60.0, max(12.0, fps)))
...
def render_pip_cards(
    ...
    *,
    ...
    from videocortex import render
...
def _overlay_filter(
    ...
    *,
    ...
    x, y, pip_w, pip_h = box
    fps_s = f"{float(src_fps):.6f}"
    # Rebuild the source clock in decode order. Passing Zoom's native
    # timestamps through overlay + `-c:a copy` is how audio dies mid-clip.
    base = f"[0:v]setpts=N/({fps_s}*TB),fps={fps_s}[base]"
    pip = f"[1:v]format=rgba,scale={pip_w}:{pip_h}:flags=lanczos"
    if not pip_cfr:
        pip += f",fps={fps_s}"
    pip += "[pip]"
    ov = (
        f"[base][pip]overlay={x}:{y}:format=auto:eof_action=repeat:shortest=1,"
    ...
    graph = f"{base};{pip};{ov}"
    if has_audio:
        graph += ";[0:a]aresample=async=1:first_pts=0[outa]"
    return graph


def _video_encode_args(*, fast: bool, crf: int, fps: float) -> list[str]:
    args = ["-r", f"{float(fps):.6f}", "-fps_mode", "cfr"]
    if fast:
        # VideoToolbox skips libx264's automatic bt709 tagging; untagged
        # 8-bit 4:2:0 gets guessed as bt601 by players, which desaturates red
        # and green — the two colours this overlay lives on.
        args += [
...
def _audio_map_args(has_audio: bool) -> list[str]:
    if not has_audio:
        return ["-an"]
    # Never `-c:a copy`. AAC-from-MOV with a negative edit-list start,
    # remuxed onto a re-encoded video, is what Safari/Chrome mute after
    # a few seconds. Re-encode onto a zero-origin clock.
    return [
        "-map", "[outa]",
        "-c:a", "aac",
        "-b:a", "192k",
...
def _duration_args(duration: float | None) -> list[str]:
    if duration is None or duration <= 0:
        return []
    return ["-t", f"{float(duration):.6f}"]
...
def compose_ffmpeg(
    *,
    ...
    if ffmpeg is None:
        raise OverlayError("ffmpeg not on PATH: brew install ffmpeg")
    src_fps = float(fps) if fps and fps > 0 else 30.0
    ov = _overlay_filter(
        box, src_fps=src_fps, pip_cfr=False, has_audio=has_audio
    )
    cmd = [
        ffmpeg, "-y",
        "-fflags", "+genpts",
        ...
        "-filter_complex", ov,
        "-map", "[outv]",
        *_audio_map_args(has_audio),
        *_video_encode_args(fast=fast, crf=crf, fps=src_fps),
        *_duration_args(duration),
...
def compose_ffmpeg_pipe(
    *,
    ...
    if ffmpeg is None:
        raise OverlayError("ffmpeg not on PATH: brew install ffmpeg")
    base_fps = float(src_fps) if src_fps and src_fps > 0 else 30.0
    ov = _overlay_filter(
        box, src_fps=base_fps, pip_cfr=True, has_audio=has_audio
    )
    cmd = [
        ffmpeg, "-y",
        "-fflags", "+genpts",
        ...
        "-map", "[outv]",
        *_audio_map_args(has_audio),
        *_video_encode_args(fast=fast, crf=crf, fps=base_fps),
        *_duration_args(duration),
    ...
    try:
        ...
        proc._videocortex_err = err  # type: ignore[attr-defined]
        return proc
    except OSError as exc:
        raise OverlayError(f"ffmpeg failed to start: {exc}") from exc
...
def _overlay_spin(
    ...
    *,
    ...
    from videocortex import spin
    from videocortex.render import blit_ribbon, compute_limits, format_pip_label
    ...
    if cfg.regions:
        try:
            from videocortex.regions import load_region_labels, top_regions_per_tr
    ...
    ribbon = None
    if cfg.ribbon:
        from videocortex.render import draw_energy_ribbon, energy_curve
    ...
    monitor = None
    if cfg.monitor:
        from videocortex.render import apply_monitor_frame
    ...
    proc = compose_ffmpeg_pipe(
        ...
        src_fps=probe["fps"], duration=probe["duration"],
    )
    ...
    try:
        ...
        try:
            ...
            pass
        err_f = getattr(proc, "_videocortex_err", None)
